...
class TestRisks(common.TransactionCase):
    ...
    def test_risk_owner_and_creator_added_to_followers_for_risk(self):
        ...
        self.assertTrue(risk.author_id.name in followers,
                        msg='Risk Author NOT in followers:%s' % risk.author_id.name)
        self.assertTrue(risk.risk_owner_id.name in followers,
                        msg='Risk Owner NOT in followers:%s' % risk.risk_owner_id.name)
        self.assertEqual('RiskTest0001', risk.name, msg='Risk name incorrect')

    def test_saving_a_change_in_users_as_followers_works(self):
        cr, uid = self.cr, self.uid
        self.risk_model.write(cr, uid, [self.test_risk_id], {'author_id': self.risk_second_author_id})
        risk = self.risk_model.browse(cr, uid, self.test_risk_id)
        followers = [follower.name for follower in risk.message_follower_ids]
    ...
    def test_adding_a_task_on_a_risk(self):
        ...
        for task in tasks:
            self.assertEqual(len(followers), len(task['message_follower_ids']),
                             msg='Incorrect number of followers set on the associated action')
            for follower in followers:
                self.assertIn(follower, task['message_follower_ids'], msg='Follower missing from follower_ids')
