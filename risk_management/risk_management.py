...
class risk_management_risk (models.Model):
    ...
    def _calculate_expected_inherent_value(self):
        for record in self:
            record.expected_value_inherent = record.impact_inherent * record.probability_inherent

    @api.depends('impact_residual', 'probability_residual')
    def _calculate_expected_residual_value(self):
        for record in self:
            record.expected_value_residual = record.impact_residual * record.probability_residual
    ...
    description = fields.Char(string='Risk Description', size=64, help="Short description of the Risk.")
    ...
    risk_category_id = fields.Many2one('risk.management.category','Risk Category',
            required=True, help="Risk Category: The type of risk in terms of the project's or business' chosen categories (e.g. Schedule, quality, legal etc.)")
    ...
    expected_value_inherent = fields.Float(compute='_calculate_expected_inherent_value', method=True, 
            string='Expected Inherent Value', store=True, help="Expected Value. Cost of inherent impact * inherent probability. This is the pre-response value.")
    expected_value_residual = fields.Float(compute='_calculate_expected_residual_value', method=True,
            string='Expected Residual Value', store=True, help="Expected Value. Cost of residual impact * residual probability. This is the post-response value.")
